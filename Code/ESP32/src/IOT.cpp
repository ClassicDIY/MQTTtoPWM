...
MQTTtoPWM::IOT _iot = MQTTtoPWM::IOT();
...
iotwebconf::ParameterGroup Device_group = iotwebconf::ParameterGroup("DeviceGroup", "SSR");
...
iotwebconf::TextParameter deviceNameParam = iotwebconf::TextParameter("SSR Name", "SSR1", _deviceName, IOTWEBCONF_WORD_LEN);
...
void onMqttMessage(char *topic, char *payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total)
{
	...
	boolean messageProcessed = _iot._pci->ProcessMQTTMessage(payload);
	if (!messageProcessed)
	{
...
void IOT::Init(SSRCommandInterface* pci)
{
	_pci = pci;
...
void IOT::Publish(const char *topic, int value, boolean retained)
{
	char buf[256];
	snprintf_P(buf, sizeof(buf), "%d", value);
	Publish(topic, buf, retained);
}
